...
def test__handle__success_records(initialize_and_clean_db, seed_site_conditions):
    ...
    site_conditions_handler = SiteConditionsHandler(
        site_condition_repo=site_condition_repo
    )

    # Act
    records = site_conditions_handler.handle(
...
def test__handle__success_no_site_id(initialize_and_clean_db):
    ...
    session = initialize_and_clean_db
    site_condition_repo = SiteConditionRepository(session=session)
    site_condition_handler = SiteConditionsHandler(
        site_condition_repo=site_condition_repo
    )
    ...
    # Act
    records = site_condition_handler.handle(
        start_date="2024-01-01", end_date="2024-01-02", site_id=None
    )
...
def test__handle__bad_dates(initialize_and_clean_db, seed_site_conditions):
    ...
    # Act
    records = site_conditions_handler.handle(
        start_date="2024-01-02", end_date="2024-01-01", site_id="TEST_ID"
    )
